//
//  ProjectsDealsController.swift
//  VGINV
//
//  Created by Zohaib on 8/12/20.
...
class ProfileProjectDealsController: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource {
    
    @IBOutlet weak var projectCollectionView: UICollectionView!
    @IBOutlet weak var addNewProject: UIButton!
    
    public static var userId = ""
    public static var isProjects = false
    ...
    override func viewDidLoad() {
        ...
        addNewProject.layer.cornerRadius = 24.0
        addNewProject.layer.borderWidth = 0
        addNewProject.layer.masksToBounds = true
        
        if (!ProfileProjectDealsController.isProjects) {
            addNewProject.backgroundColor = UIColor.red
            addNewProject.setTitle("AddNewDeal".l10n(), for: .normal)
//            projectHeading.text = NSLocalizedString("Deals", comment: "")
        } else {
            addNewProject.setTitle("AddNewProject".l10n(), for: .normal)
//            projectHeading.text = NSLocalizedString("projects", comment: "")
        }
...
//                self.userId = id
//            }
//            if let project = dict["isProject"] as? Bool{
//                self.isProjects = project
//            }
//        }
//    }
        ...
        if navigationController != nil{
            ...
            self.addCreateGroup()
             if (!ProfileProjectDealsController.isProjects) {
                ...
                set(title: "project".l10n(), mode: .automatic)
            }
        }
    }
...
//                    }
    ...
    private func fetchUserProfile(){
        ...
        CloudDataService.sharedInstance.getProfile(ConstantStrings.PROFILE_URL, success: { (json) in
            ...
            if (ProfileProjectDealsController.self.isProjects) {
                if (json.projects!.count > 0) {
                    for project in json.projects! {
                        if (project.auth?.description == ProfileProjectDealsController.self.userId) {
                ...
                if (json.deals!.count > 0) {
                    for project in json.deals! {
                        if (project.auth?.description == ProfileProjectDealsController.self.userId) {
                            self.dealsItems.append(project)
                        }
                    }
                } else {
...
extension ProfileProjectDealsController: Configurable {
    func setup() {
...
private extension ProfileProjectDealsController {
    func reload() {
