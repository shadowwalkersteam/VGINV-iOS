...
public class CometChatGroupList: UIViewController, UINavigationControllerDelegate {
...
//            //
//            //            }
//            let group : Group = Group(guid: "1", name: "All Members", groupType: CometChat.groupType.public, password: "")
...
//        })
        ...
        if (DashboardViewController.isMembersChat) {
            groupRequest = GroupsRequest.GroupsRequestBuilder(limit: 50).set(searchKeyword: Defaults.readString(key: Defaults.USER_TYPE)).set(joinedOnly: true).build()
        } else {
        ...
        groupRequest!.fetchNext(onSuccess: { (groups) in
            ...
            if groups.count != 0{
                if (DashboardViewController.isMembersChat) {
                    let joinedGroups = groups.filter({$0.hasJoined == true && $0.guid == Defaults.readString(key: Defaults.USER_TYPE)})
                    self.groups.append(contentsOf: joinedGroups)
                } else {
                    let joinedGroups = groups.filter({$0.hasJoined == true && !($0.name?.contains("HMG"))! && !($0.name?.contains("VG"))!})
                    self.groups.append(contentsOf: joinedGroups)
                }
                DispatchQueue.main.async {
                    self.activityIndicator?.stopAnimating()
...
extension CometChatGroupList : UISearchBarDelegate, UISearchResultsUpdating {
    ...
    */
    public func updateSearchResults(for searchController: UISearchController) {
        if (DashboardViewController.isMembersChat) {
            groupRequest  = GroupsRequest.GroupsRequestBuilder(limit: 50).set(searchKeyword: Defaults.readString(key: Defaults.USER_TYPE)).set(joinedOnly: true).set(searchKeyword: searchController.searchBar.text ?? "").build()
        } else {
            groupRequest  = GroupsRequest.GroupsRequestBuilder(limit: 50).set(joinedOnly: true).set(searchKeyword: searchController.searchBar.text ?? "").build()
        }
        groupRequest!.fetchNext(onSuccess: { (groups) in
            print("fetchGroups onSuccess: \(groups)")
            if groups.count != 0{
                if (DashboardViewController.isMembersChat) {
                    let joinedGroups = groups.filter({$0.hasJoined == true && $0.guid == Defaults.readString(key: Defaults.USER_TYPE)})
                    self.filteredGroups = joinedGroups
                } else {
                    let joinedGroups = groups.filter({$0.hasJoined == true && !($0.name?.contains("HMG"))! && !($0.name?.contains("VG"))!})
                    self.filteredGroups = joinedGroups
                }
