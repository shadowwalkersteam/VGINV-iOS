...
public class CometChatConversationList: UIViewController {
    ...
    var conversationRequest = ConversationRequest.ConversationRequestBuilder(limit: 100).setConversationType(conversationType: .user).build()
    ...
    private func refreshConversations(){
        ...
        conversationRequest = ConversationRequest.ConversationRequestBuilder(limit: 30).setConversationType(conversationType: .user).build()
    ...
    private func addCreateGroup(_ inNavigationBar: Bool){
        if inNavigationBar == true {
            let startChat = #imageLiteral(resourceName: "createGroup")
            let startChatButton = UIBarButtonItem(image: startChat, style: .plain, target: self, action: #selector(startNewChat))
...
extension CometChatConversationList: UITableViewDelegate , UITableViewDataSource {
    ...
    public func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        ...
        if conversations.isEmpty {
            self.tableView.setEmptyMessage("No Chats Found.".l10n())
        } else{
