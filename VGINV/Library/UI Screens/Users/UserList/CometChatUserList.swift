...
public class CometChatUserList: UIViewController {
    ...
    var userRequest = UsersRequest.UsersRequestBuilder()
        .set(limit: 20)
        .set(role : Defaults.readString(key: Defaults.USER_TYPE))
        .build();
    ...
    private func refreshUsers(){
        ...
        tableView.tableFooterView = activityIndicator
        tableView.tableFooterView?.isHidden = false
        userRequest = UsersRequest.UsersRequestBuilder()
        .set(limit: 20)
        .set(role : Defaults.readString(key: Defaults.USER_TYPE))
        .build();
        userRequest.fetchNext(onSuccess: { (users) in
...
extension CometChatUserList : UISearchBarDelegate, UISearchResultsUpdating {
    ...
    public func updateSearchResults(for searchController: UISearchController) {
        userRequest = UsersRequest.UsersRequestBuilder()
        .set(limit: 20)
        .set(role : Defaults.readString(key: Defaults.USER_TYPE)).set(searchKeyword: searchController.searchBar.text ?? "").build()
        userRequest.fetchNext(onSuccess: { (users) in
            if users.count != 0 {
